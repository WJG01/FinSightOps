...
PERSISTENCE (2026-08-10): calculate_pnl (natural-language ad-hoc queries)
writes a period-keyed record (PK="pnl#{start}_{end}", SK="statement").

RUN-KEYED PATH ADDED (2026-08-10): Reconciliation's design expects one
trial_balance + one pnl + one balance_sheet per run_id, all under
PK=f"run#{run_id}" — matching ledger's own per-run trial_balance. This adds
a SEPARATE deterministic path (calculate_pnl_for_run) for that automated
flow: no Bedrock call, pure Python math, Query instead of Scan (cheaper,
scoped to one run's entries). Triggered with {"run_id": "..."} instead of
{"query": "..."}. The natural-language Strands path is untouched — it's a
different consumer (a human asking an ad-hoc question) with a different
need (prose, not a number reconciliation can key off).
  table.get_item(Key={"PK": f"run#{run_id}", "SK": "pnl"})
...
IAM Permissions needed:
  - dynamodb:Scan, dynamodb:Query, dynamodb:PutItem on the table
...
# ===========================================================================
# RUN-KEYED PATH — for the automated pipeline (reconciliation), not Strands.
# No Bedrock call. Query, not Scan. Deterministic math only.
# ===========================================================================

def get_entries_for_run(run_id: str) -> list[dict]:
    """Query is scoped to one run's own entries — cheaper and simpler than
    a table-wide Scan with a date filter."""
    items = []
    key_expr = Key("PK").eq(f"run#{run_id}") & Key("SK").begins_with("entry#")
    resp = table.query(KeyConditionExpression=key_expr)
    items.extend(resp.get("Items", []))
    while "LastEvaluatedKey" in resp:
        resp = table.query(KeyConditionExpression=key_expr, ExclusiveStartKey=resp["LastEvaluatedKey"])
        items.extend(resp.get("Items", []))
    return decimals_to_floats(items)


def save_pnl_for_run(run_id: str, result: dict) -> None:
    """Persists at PK=run#{run_id}, SK=pnl — the exact shape reconciliation's
    _get(run_id, "pnl") expects."""
    def to_strs(d: dict) -> dict:
    ...
    table.put_item(Item={
        ...
        "created_at":     datetime.now(timezone.utc).isoformat(),
        "revenue_items":  to_strs(result["revenue_items"]),
        "cogs_items":     to_strs(result["cogs_items"]),
        "expense_items":  to_strs(result["expense_items"]),
        "total_revenue":  str(result["total_revenue"]),
        "total_cogs":     str(result["total_cogs"]),
        "total_expenses": str(result["total_expenses"]),
        "total_operating_expenses": str(result["total_operating_expenses"]),
        "gross_profit":   str(result["gross_profit"]),
        "net_income":     str(result["net_income"]),
        "transaction_count": result["transaction_count"],
    })
    logger.info(f"P&L saved for run_id={run_id}: net_income={result['net_income']}")


def calculate_pnl_for_run(run_id: str) -> dict:
    """
    The deterministic, non-agent P&L calculation used by the automated
    pipeline (runner / reconciliation), keyed to a single run_id rather
    than a date range. Same aggregation and classification logic as the
    Strands-facing calculate_pnl tool below — just scoped differently.
    """
    ...
    for code, t in totals.items():
        ...
        net_credit = round(t["credit"] - t["debit"], 2)
        net_debit  = round(t["debit"] - t["credit"], 2)
    ...
    period = {"start": dates[0], "end": dates[-1]} if dates else {"start": None, "end": None}
    ...
    result = {
        ...
        # Alias — reconciliation's tie-out checks (expenses_tie_to_ledger,
        # pnl_arithmetic) read total_operating_expenses specifically. Same
        # value, kept under both names so neither consumer breaks.
    ...
    save_pnl_for_run(run_id, result)
...
def lambda_handler(event, context):
    """
    AWS Lambda entry point. Two independent capabilities, routed by which
    key is present:

      {"run_id": "run-007"}
        -> deterministic path, no Bedrock, Query not Scan. Persists at
           PK=run#{run_id}, SK=pnl. This is what reconciliation depends on.

      {"query": "Generate a P&L for Q1 2026"}
        -> existing Strands natural-language path, unchanged. For ad-hoc
           human questions — persists a separate period-keyed record.
    """
    ...
    try:
        ...
        if run_id and not query:
            result = calculate_pnl_for_run(run_id)
        ...
        if not query:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Provide either 'run_id' (automated) or 'query' (natural language)"}),
            }
